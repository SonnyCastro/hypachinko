...
import { useMachineGameState } from "@/hooks/useMachineGameState"
import {
  TokenSelector,
  UpForGrabsDisplay,
  RecentActivityFeed,
...
export function HomePage() {
  ...
  const [selectedToken, setSelectedToken] = useState("usdt0")

  const { handlePercentageSelect, handleBallCountChange } = useGameLogic()

  // Use the machine game state hook for real contract integration
  const machineGameState = useMachineGameState("home")

  const handlePercentageSelectWrapper = (percentage: string) => {
    const result = handlePercentageSelect(percentage)
    setSelectedPercentage(result.percentage)
    setBallCount(result.ballCount)
  }

  const handleBallCountChangeWrapper = (count: number) => {
    const result = handleBallCountChange(count)
    setBallCount(result.ballCount)
    setSelectedPercentage(result.percentage)
  }

  // Use the real contract function from the hook
  const handleBuyBalls = async () => {
    try {
      await machineGameState.buyTicket()
      console.log(
        `Successfully bought ${ballCount} balls for ${selectedToken} at ${selectedPercentage}%`
      )
    } catch (error) {
          ...
          <div className='flex flex-col gap-6 sm:gap-8 lg:gap-[38px] min-h-[500px] sm:min-h-[600px] lg:min-h-[700px] items-center justify-center relative w-full py-8 sm:py-12 lg:py-16 px-4 sm:px-6 lg:px-8'>
            ...
            <div className='flex flex-col gap-3 sm:gap-4 lg:gap-2.5 items-start justify-start w-full max-w-[320px] sm:max-w-[400px] md:max-w-[480px] lg:max-w-[544px] relative z-10'>
              ...
              <MachineCard
                ...
                onPercentageSelect={handlePercentageSelectWrapper}
                onBallCountChange={handleBallCountChangeWrapper}
              />
            </div>
